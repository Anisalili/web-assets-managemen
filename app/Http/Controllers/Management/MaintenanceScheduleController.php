...
class MaintenanceScheduleController extends Controller
{
    ...
    public function index(Request $request): View
    {
        ...
        $filters['frequency'] = is_array($filters['frequency']) ? $filters['frequency'] : [$filters['frequency']];
        ...
        $hasFilters = !empty($filters['search']) || !empty($filters['asset_id']) || !empty($filters['status']) || !empty($filters['frequency']) || !empty($filters['date_from']) || !empty($filters['date_to']);
        ...
        return view('management.maintenance-schedules.index', compact('schedules', 'search', 'assetId', 'status', 'frequency', 'dateFrom', 'dateTo', 'assets', 'statuses', 'frequencies'));
    }
    ...
    public function create(): View
    {
        ...
        return view('management.maintenance-schedules.create', compact('assets', 'statuses', 'frequencies'));
    }
    ...
    {
        $maintenanceSchedule->load(['asset.category', 'asset.room.building', 'logs']);

        return view('management.maintenance-schedules.show', compact('maintenanceSchedule'));
    }
    ...
    public function edit(MaintenanceSchedule $maintenanceSchedule): View
    {
        ...
        return view('management.maintenance-schedules.edit', compact('maintenanceSchedule', 'assets', 'statuses', 'frequencies'));
    }
    ...
    {
        try {
            $this->scheduleService->updateSchedule($maintenanceSchedule, $request->validated());
            ...
            return back()
                ->withInput()
                ->with('error', 'Gagal memperbarui jadwal pemeliharaan: ' . $e->getMessage());
        }
    }
    ...
    public function destroy(MaintenanceSchedule $maintenanceSchedule): RedirectResponse
    {
        try {
            ...
            return back()->with('error', 'Gagal menghapus jadwal pemeliharaan: ' . $e->getMessage());
        }
    }
}
